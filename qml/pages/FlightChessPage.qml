...
Page {
    ...
    readonly property bool inNetworkGame: AppCtrl.networkManager.isHost
                                          || AppCtrl.networkManager.isConnected
    readonly property int localPlayer: !inNetworkGame ? AppCtrl.flightChessController.currentPlayer
                                                       : (AppCtrl.networkManager.isHost ? 1 : 2)
    readonly property bool localTurn: !inNetworkGame
                                      || AppCtrl.flightChessController.currentPlayer === localPlayer
    ...
    property bool pendingNetworkRoll: false
    ...
    function edgeMiddlePoint(edge) {
        if (edge === 0)
            return gridPoint(1, 7)
        if (edge === 1)
            return gridPoint(7, 13)
        if (edge === 2)
            return gridPoint(13, 7)
        return gridPoint(7, 1)
    }
    ...
    function launchPoint(player) {
        if (player === 1)
            return gridPoint(12, 5)
        return gridPoint(2, 9)
    }
    ...
    function drawBoardDot(ctx, x, y, radius, fillColor, strokeColor, highlight) {
        ctx.save()
        ctx.shadowColor = highlight ? "rgba(0, 0, 0, 0.18)" : "rgba(0, 0, 0, 0.08)"
        ctx.shadowBlur = highlight ? 5 : 2
    ...
    function orthogonalMovePath(rawPath) {
        ...
        for (var i = 1; i < rawPath.length; ++i) {
            ...
            if (dx > 1 && dy > 1) {
                var corner = Qt.point(next.x, previous.y)
                path.push(corner)
            }
    ...
    ColumnLayout {
        ...
        RowLayout {
            ...
            Rectangle {
                ...
                Rectangle {
                    ...
                    property int value: AppCtrl.flightChessController.diceValue
        ...
        Timer {
            ...
            onTriggered: {
                ...
                if (root.pendingNetworkRoll) {
                    root.pendingNetworkRoll = false
                    AppCtrl.networkManager.sendFlightRoll()
                    return
                }

                var rollingPlayer = AppCtrl.flightChessController.currentPlayer
                var diceValue = AppCtrl.flightChessController.rollDice()
                if (root.inNetworkGame && diceValue > 0)
                    AppCtrl.networkManager.broadcastFlightRoll(rollingPlayer, diceValue)
            }
        }
            ...
            Canvas {
                ...
                onPaint: {
                    ...
                    for (var j = 0; j < planes.length; ++j) {
                        ...
                        var scale = plane.inBase ? (plane.finished ? 0.22 : 0.28)
                                                 : (plane.inLaunch ? 0.46 : (plane.canMove ? 0.62 : 0.55))
                    ...
                    onClicked: function(mouse) {
                        ...
                        for (var i = 0; i < planes.length; ++i) {
                            ...
                            if (Math.sqrt(dx * dx + dy * dy) <= 20) {
                                if (root.inNetworkGame && !AppCtrl.networkManager.isHost) {
                                    AppCtrl.networkManager.sendFlightMove(plane.index)
                                } else {
                                    var movingPlayer = AppCtrl.flightChessController.currentPlayer
                                    root.pendingMoveStart = root.planeSnapshot(plane)
                                    if (AppCtrl.flightChessController.movePlane(plane.index)) {
                                        if (root.inNetworkGame)
                                            AppCtrl.networkManager.broadcastFlightMove(movingPlayer, plane.index)
                                    } else {
                                        root.pendingMoveStart = null
                                    }
                                }
        ...
        ActionButton {
            ...
            enabled: AppCtrl.flightChessController.gameOver
                     || (root.localTurn && !AppCtrl.flightChessController.hasRolled && !root.diceRolling)
            onClicked: {
                if (AppCtrl.flightChessController.gameOver) {
                    AppCtrl.flightChessController.startNewGame()
                } else if (root.inNetworkGame && !AppCtrl.networkManager.isHost) {
                    root.pendingNetworkRoll = true
        ...
        RowLayout {
            ...
            ActionButton {
                ...
                text: "保存存档"
                secondary: true
                enabled: !root.diceRolling && !root.moveAnimating
                onClicked: AppCtrl.flightChessController.saveGame()
            }
            ...
            ActionButton {
                ...
                text: "读取存档"
                secondary: true
                enabled: AppCtrl.flightChessController.hasSavedGame
                         && !root.diceRolling
                         && !root.moveAnimating
                onClicked: {
                    root.pendingMoveStart = null
                    root.moveAnimating = false
                    moveStepTimer.stop()
                    AppCtrl.flightChessController.loadGame()
                }
            }
        }
